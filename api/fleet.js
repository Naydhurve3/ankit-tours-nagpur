...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      // Public: only visible
      const rows = await sql`SELECT id, name, seating, price, features, image, visible, created_at FROM fleet WHERE visible=true ORDER BY created_at DESC`;
    ...
    if (req.method === 'POST') {
      ...
      const rows = await sql`INSERT INTO fleet (name,seating,price,features,image,visible) VALUES (${n},${sanitize(seating,40)},${sanitize(price,40)},${sanitize(features,120)},${sanitize(image,500)},${visible!==false}) RETURNING *`;
      await logAudit(sql,{event_type:'CREATE', entity_type:'fleet', entity_id:rows[0].id, after_json:rows[0], req});
      return res.status(201).json(rows[0]);
    }
    if (req.method === 'PUT') {
      const { id, name, seating, price, features, image, visible } = req.body || {};
      ...
      const rows = await sql`UPDATE fleet SET name=${sanitize(name,80)}, seating=${sanitize(seating,40)}, price=${sanitize(price,40)}, features=${sanitize(features,120)}, image=${sanitize(image,500)}, visible=${visible} WHERE id=${id} RETURNING *`;
