...
function isValidUrl(u){
  ...
  if(u.startsWith('data:')) return /^data:image\/(png|jpe?g|webp);base64,/.test(u) && u.length<=400000; // base64 stored in Neon
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if(!isValidUrl(image)) return res.status(400).json({ error: 'image must be https URL or base64' });
      const rows = await sql`INSERT INTO fleet (name,seating,price,features,image,display_mode,model_color,vehicle_type,visible) VALUES (${n},${sanitize(seating,40)},${sanitize(price,40)},${sanitize(features,120)},${sanitize(image,400000)},${displayMode(display_mode)},${modelColor(model_color)},${vehicleType(vehicle_type)},${visible!==false}) RETURNING *`;
    ...
    if (req.method === 'PUT') {
      ...
      const rows = await sql`UPDATE fleet SET name=${sanitize(name,80)}, seating=${sanitize(seating,40)}, price=${sanitize(price,40)}, features=${sanitize(features,120)}, image=${sanitize(image,400000)}, display_mode=${displayMode(display_mode)}, model_color=${modelColor(model_color)}, vehicle_type=${vehicleType(vehicle_type)}, visible=${visible}, updated_at=now() WHERE id=${id} RETURNING *`;
