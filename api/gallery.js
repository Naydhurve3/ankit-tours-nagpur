...
function isUrl(u){ if(!u) return false; if(u.startsWith('data:')) return false; try{ const p=new URL(u); return ['https:','http:'].includes(p.protocol);}catch{ return false; } }
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='POST'){ const {src,category,title,visible}=req.body||{}; if(!src || !isUrl(src)) return res.status(400).json({error:'valid https src required, base64 not allowed'}); const rows=await sql`INSERT INTO gallery (src,category,title,visible) VALUES (${s(src,500)},${s(category,20)},${s(title,80)},${visible!==false}) RETURNING *`; await logAudit(sql,{event_type:'CREATE', entity_type:'gallery', entity_id:rows[0].id, after_json:rows[0], req}); return res.status(201).json(rows[0]); }
    if(req.method==='PUT'){ const {id,src,category,title,visible}=req.body||{}; if(!id) return res.status(400).json({error:'id required'}); if(src && !isUrl(src)) return res.status(400).json({error:'invalid src'}); const before=await sql`SELECT * FROM gallery WHERE id=${id} LIMIT 1`; const rows=await sql`UPDATE gallery SET src=${s(src,500)},category=${s(category,20)},title=${s(title,80)},visible=${visible} WHERE id=${id} RETURNING *`; await logAudit(sql,{event_type:'UPDATE', entity_type:'gallery', entity_id:id, before_json:before[0], after_json:rows[0], req}); return res.status(200).json(rows[0]); }
